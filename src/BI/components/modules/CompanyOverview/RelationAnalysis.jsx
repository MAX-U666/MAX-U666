  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#1a1f2e] rounded-xl p-6">
        <h3 className="text-white font-medium mb-4">指标相关性分析</h3>
            ...
            return (
              <div
                ...
                className={`p-3 rounded-lg cursor-pointer transition-all ${
                  selectedPair === item ? "bg-[#252b3d] ring-1 ring-blue-500/30" : "hover:bg-[#252b3d]"
                }`}
      ...
      {/* 散点图模拟 */}
      <div className="bg-[#1a1f2e] rounded-xl p-6">
        <h3 className="text-white font-medium mb-4">广告花费 vs 订单量（按店铺）</h3>
        <div className="relative h-64 border-l border-b border-gray-700">
          {/* Y轴标签 */}
          ...
          {scatterData.map((point, idx) => {
            ...
            return (
              <div
                key={idx}
                className="absolute w-4 h-4 rounded-full bg-blue-500 hover:scale-150 transition-transform cursor-pointer group"
                style={{
                  left: `${x}%`,
                  bottom: `${y}%`,
              ...
              >
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-[#252b3d] rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="text-white font-medium">{point.shop}</div>
                  <div className="text-gray-400">广告: {formatCNY(point.adSpend)}</div>
                  <div className="text-gray-400">订单: {point.orders}</div>
                </div>
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#1a1f2e] rounded-xl p-4">
          <div className="text-gray-400 text-sm mb-2">关键发现</div>
          <p className="text-white text-sm">
            广告花费与订单量呈强正相关（r=0.85），每增加1000元广告投入，预计可带来约28单增量。
          </p>
        </div>
        <div className="bg-[#1a1f2e] rounded-xl p-4">
          <div className="text-gray-400 text-sm mb-2">优化建议</div>
          <p className="text-white text-sm">
