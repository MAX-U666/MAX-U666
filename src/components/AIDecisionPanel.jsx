...
const AIDecisionPanel = ({ selectedProduct, currentDayData, currentDay, onExecute, onAbnormal, currentUser }) => {
  ...
  const getDecisionColor = (decision) => {
    ...
    return colors[decision] || '#86868B';
  };

  const getPhaseColor = (phase) => {
    const colors = { 'A': '#F59E0B', 'B': '#3B82F6', 'C': '#10B981' };
    return colors[phase] || '#86868B';
  };
  ...
  const renderFullReport = (report) => {
    ...
    for (let i = 0; i < lines.length; i++) {
              ...
              <div key={`action-${codeBlockKey++}`} style={{ 
                ...
                {actionCardContent.map((cardLine, j) => {
                  ...
                  if (cl.startsWith('①') || cl.startsWith('②') || cl.startsWith('③')) {
                    return <div key={j} style={{ color: '#6E6E73', fontSize: '12px', margin: '2px 0', paddingLeft: '20px' }}>{cl}</div>;
                  }
                  if (cl) {
                    return <div key={j} style={{ color: '#1D1D1F', fontSize: '13px', margin: '4px 0', paddingLeft: '8px' }}>{cl}</div>;
                  }
                  return null;
                })}
              </div>
              ...
              {codeContent.map((codeLine, j) => (
                <div key={j} style={{ 
                  color: codeLine.trim().startsWith('✅') ? '#10B981' : 
                         ...
                         codeLine.trim().startsWith('【') ? '#FF6B35' : '#1D1D1F'
                }}>{codeLine}</div>
              ))}
            </div>
          );
      ...
      // 引用块
      if (line.startsWith('> ')) {
        result.push(<div key={i} style={{ borderLeft: '3px solid #3B82F6', paddingLeft: '12px', margin: '8px 0', color: '#6E6E73', fontStyle: 'italic' }}>{line.replace('> ', '')}</div>);
        continue;
      }
      // 列表项
      if (trimmedLine.startsWith('- ') || trimmedLine.startsWith('• ') || trimmedLine.startsWith('* ')) {
        ...
        result.push(
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', margin: '6px 0', color: isError ? '#F87171' : isSuccess ? '#10B981' : '#CBD5E1' }}>
            <span style={{ color: isError ? '#EF4444' : isSuccess ? '#10B981' : '#86868B' }}>•</span>
            <span>{content}</span>
          </div>
        );
        continue;
      ...
      if (line.includes('**')) {
        ...
        result.push(
          <p key={i} style={{ margin: '8px 0', color: '#1D1D1F', lineHeight: '1.8' }}>
  ...
  const TabButton = ({ id, icon, label, color }) => (
    <button
      ...
      style={{
        ...
        background: activeTab === id ? `${color}15` : 'rgba(0,0,0,0.02)',
        color: activeTab === id ? color : '#6E6E73',
      ...
      case 'judgment':
        return (
          <div style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '12px', padding: '16px' }}>
            <h5 style={{ margin: '0 0 12px 0', color: '#6E6E73', fontSize: '12px' }}>🔍 核心卡点</h5>
            ...
            )) : <div style={{ color: '#86868B', fontSize: '12px' }}>详见完整分析报告</div>}
          </div>
        );
      case 'strategy':
        return (
          <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '12px', padding: '16px' }}>
            <h5 style={{ margin: '0 0 12px 0', color: '#6E6E73', fontSize: '12px' }}>✅ 今日必做</h5>
      ...
      case 'risk':
        return (
          <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '12px', padding: '16px' }}>
            <h5 style={{ margin: '0 0 12px 0', color: '#6E6E73', fontSize: '12px' }}>❌ 禁止操作</h5>
      ...
      case 'time':
        return (
          <div style={{ background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: '12px', padding: '16px' }}>
            <h5 style={{ margin: '0 0 12px 0', color: '#6E6E73', fontSize: '12px' }}>⏰ 盯盘时间</h5>
            {observationTimes.length > 0 ? observationTimes.map((item, i) => (
              <div key={i} style={{ fontSize: '12px', color: '#FBBF24', marginBottom: '10px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                <span>⏰</span> <span style={{ color: '#1D1D1F' }}>{item.replace(/^⏰\s*/, '')}</span>
              </div>
            )) : <div style={{ color: '#86868B', fontSize: '12px' }}>详见完整分析报告</div>}
          </div>
        );
            ...
            {analysis.idn_enhancement ? (
              <>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '12px', color: '#1D1D1F', lineHeight: '1.6' }}>{analysis.idn_enhancement.key_insight}</p>
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <h5 style={{ margin: '0 0 8px 0', color: '#60A5FA', fontSize: '12px' }}>📦 物流建议</h5>
                  <p style={{ margin: 0, fontSize: '12px', color: '#1D1D1F', lineHeight: '1.6' }}>{analysis.idn_enhancement.logistics_note}</p>
                </div>
                <div>
                  <h5 style={{ margin: '0 0 8px 0', color: '#34D399', fontSize: '12px' }}>🌏 本地化提示</h5>
                  <p style={{ margin: 0, fontSize: '12px', color: '#1D1D1F', lineHeight: '1.6' }}>{analysis.idn_enhancement.localization_tip}</p>
                </div>
              </>
            ) : <div style={{ color: '#86868B', fontSize: '12px' }}>详见完整分析报告中的印尼专属增强模块</div>}
          </div>
        );
      default:
        return null;
  ...
  if (!currentDayData || currentDayData.status === '未提交') {
    return (
      <div style={styles.card}>
        <div style={{ background: 'linear-gradient(135deg, #F5F5F7 0%, #FFFFFF 100%)', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '12px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <span style={{ fontSize: '14px', fontWeight: '700', color: '#1D1D1F' }}>Day {currentDay} AI决策</span>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={styles.card}>
          <div 
            ...
            style={{ 
              ...
              alignItems: 'center', 
              borderBottom: reportExpanded ? '1px solid rgba(0,0,0,0.06)' : 'none',
              cursor: 'pointer'
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: '700', color: '#1D1D1F' }}>AI 完整分析报告</span>
              {analysisSource && (
            ...
            <span style={{ color: '#86868B', fontSize: '20px', transition: 'transform 0.2s', transform: reportExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>▼</span>
          </div>
          ...
          {reportExpanded && (
            <div style={{ padding: '20px', maxHeight: '600px', overflowY: 'auto' }}>
              {analysis?.full_report ? (
                ...
                <div style={{ color: '#86868B', textAlign: 'center', padding: '40px' }}>
                  暂无完整分析报告
                </div>
              )}
            </div>
      ...
      {/* AI决策执行面板 - 放在后面 */}
      <div style={styles.card}>
        <div style={{ background: 'linear-gradient(135deg, #F5F5F7 0%, #FFFFFF 100%)', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ 
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: isExecuted ? 'repeat(4, 1fr)' : '1fr 1fr 1fr auto', gap: '12px', marginBottom: '16px' }}>
                <div style={{ background: `${getPhaseColor(analysis?.phase)}15`, border: `1px solid ${getPhaseColor(analysis?.phase)}40`, borderRadius: '12px', padding: '12px', textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', color: '#86868B', marginBottom: '4px' }}>阶段</div>
                  <div style={{ fontSize: '18px', fontWeight: '700', color: getPhaseColor(analysis?.phase) }}>阶段 {analysis?.phase || 'A'}</div>
                  <div style={{ fontSize: '10px', color: '#86868B' }}>{analysis?.phase_name}</div>
                </div>
                <div style={{ background: `${getDecisionColor(analysis?.today_decision || currentDayData.ai_action)}15`, border: `1px solid ${getDecisionColor(analysis?.today_decision || currentDayData.ai_action)}40`, borderRadius: '12px', padding: '12px', textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', color: '#86868B', marginBottom: '4px' }}>{isExecuted ? '执行决策' : '今日判断'}</div>
                  ...
                  <div style={{ fontSize: '10px', color: '#86868B' }}>置信度 {analysis?.confidence || currentDayData.ai_confidence}%</div>
                </div>
                <div style={{ background: `${getSupplementColor(analysis?.supplement_strategy)}15`, border: `1px solid ${getSupplementColor(analysis?.supplement_strategy)}40`, borderRadius: '12px', padding: '12px', textAlign: 'center' }}>
                  <div style={{ fontSize: '10px', color: '#86868B', marginBottom: '4px' }}>补单策略</div>
                ...
                {isExecuted && (
                  <div style={{ background: 'rgba(100,116,139,0.1)', border: '1px solid rgba(100,116,139,0.3)', borderRadius: '12px', padding: '12px', textAlign: 'center' }}>
                    <div style={{ fontSize: '10px', color: '#86868B', marginBottom: '4px' }}>ROI</div>
