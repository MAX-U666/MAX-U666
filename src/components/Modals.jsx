...
export const NewProductModal = ({ newProduct, setNewProduct, onClose, onCreate, currentUser }) => (
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
    <div style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #F5F5F7 100%)', borderRadius: '24px', width: '500px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
...
export const UploadModal = ({ 
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #F5F5F7 100%)', borderRadius: '24px', width: '800px', maxHeight: '90vh', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
...
export const AbnormalModal = ({ abnormalReason, setAbnormalReason, onClose, onSubmit }) => (
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
    <div style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #F5F5F7 100%)', borderRadius: '20px', padding: '28px', width: '420px', border: '1px solid rgba(255,255,255,0.1)' }}>
      <h3 style={{ margin: '0 0 20px 0', fontSize: '18px', color: '#E2E8F0', fontWeight: '700' }}>上报异常</h3>
      <textarea value={abnormalReason} onChange={(e) => setAbnormalReason(e.target.value)} placeholder="请说明异常原因..." style={{ width: '100%', height: '120px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '14px', fontSize: '14px', color: '#E2E8F0', resize: 'none', outline: 'none', boxSizing: 'border-box' }} />
...
const InputField = ({ label, type = 'text', value, onChange, placeholder, step }) => (
  <div style={{ marginBottom: '20px' }}>
    ...
    <input type={type} step={step} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} style={{ width: '100%', padding: '14px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', fontSize: '14px', color: '#E2E8F0', outline: 'none', boxSizing: 'border-box' }} />
  </div>
);
    ...
    <div onClick={() => fileRef.current?.click()} style={{ border: `2px dashed ${color}40`, borderRadius: '14px', padding: '28px', textAlign: 'center', cursor: 'pointer', background: data ? `${color}10` : 'rgba(255,255,255,0.02)', minHeight: '100px' }}>
      ...
      {data ? (
        ...
        <div>
          ...
          <div style={{ fontSize: '12px', color: '#64748B' }}>点击上传</div>
        </div>
      )}
    </div>
  </div>
...
const DataPreviewFull = ({ sku, matchedShop, matchedAd }) => {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '14px', padding: '18px', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      {matchedAd && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px' }}>
            {adItems.map((item, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.05)', padding: '10px', borderRadius: '8px', textAlign: 'center' }}>
