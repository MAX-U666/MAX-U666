      ...
      <div style={styles.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <GMVLogo size={44} />
            <div>
              <h1 style={{ margin: 0, fontSize: '18px', fontWeight: '800', color: '#1C1C1E' }}>Shopee GMV MAX</h1>
              <p style={{ margin: 0, fontSize: '11px', color: '#8E8E93' }}>AI决策中枢 · 系统博弈专家</p>
            </div>
          </div>
          {currentView === 'detail' && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div style={{ background: 'rgba(255,107,53,0.1)', border: '1px solid rgba(255,107,53,0.2)', borderRadius: '12px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontSize: '10px', color: '#8E8E93' }}>数据截止</div>
          ...
          {selectedProduct && currentView === 'detail' && (
            <div style={{ background: '#E5E5EA', borderRadius: '12px', padding: '10px 16px', border: '1px solid #D1D1D6' }}>
              <span style={{ fontSize: '11px', color: '#8E8E93' }}>执行 </span>
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowUserMenu(!showUserMenu)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px 14px', borderRadius: '12px', border: '1px solid #E5E5EA', background: '#F2F2F7', color: '#fff', cursor: 'pointer' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                ...
                <div style={{ fontSize: '10px', color: '#8E8E93' }}>{currentUser?.role === 'admin' ? '管理员' : '运营'}</div>
              </div>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#8E8E93" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            ...
            {showUserMenu && (
              <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: 'linear-gradient(180deg, #FFFFFF 0%, #F5F5F7 100%)', borderRadius: '14px', border: '1px solid #E5E5EA', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', padding: '10px', minWidth: '220px', zIndex: 1000 }}>
                <div style={{ padding: '10px 14px', fontSize: '11px', color: '#8E8E93', borderBottom: '1px solid #D1D1D6', marginBottom: '8px' }}>切换用户</div>
                {users.map(user => (
                  <button key={user.id} onClick={() => { setCurrentUser(user); setShowUserMenu(false); setFilterOwner('mine'); }} style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '12px 14px', borderRadius: '10px', border: 'none', background: currentUser?.id === user.id ? 'rgba(255,107,53,0.1)' : 'transparent', cursor: 'pointer', textAlign: 'left' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#1C1C1E' }}>{user.name}</div>
                      <div style={{ fontSize: '10px', color: '#8E8E93' }}>{user.role === 'admin' ? '管理员' : '运营'}</div>
                    </div>
